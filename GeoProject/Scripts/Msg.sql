UPDATE GEO.EMP
SET emp_pw = '1234', EMP_STATUS = 'ST001'
WHERE EMP_NO = 'EE001';
...
INSERT INTO NOTI(NOTI_NO, EMP_NO, NOTI_STATUS,
	...
	SELECT 12 + 0, 'EE001', '2', '알림 내용', SYSDATE, 'SYSTEM', SYSDATE, 'SYSTEM', SYSDATE FROM DUAL
	UNION ALL
	SELECT 12 + 1, 'EE001', '2', '알림 내용', SYSDATE, 'SYSTEM', SYSDATE, 'SYSTEM', SYSDATE FROM DUAL
	UNION ALL
	SELECT 12 + 2, 'EE001', '2', '알림 내용', SYSDATE, 'SYSTEM', SYSDATE, 'SYSTEM', SYSDATE FROM DUAL;

SELECT NOTI_NO , EMP.EMP_NAME AS EMP_NO , NOTI_STATUS ,
	NOTI_CONTENT , NOTI_DATE , NOTI_READ_YN
	FROM (
		SELECT NOTI_NO , EMP_NO , NOTI_STATUS ,
			NOTI_CONTENT , NOTI_DATE , NOTI_READ_YN 
			FROM NOTI
			WHERE EMP_NO = 'EE001'
				AND NOTI_READ_YN = 'N'
...
CREATE TABLE "NOTI" (
	...
	"ORIGIN_NO"	VARCHAR2(100)		NOT NULL,
	"NOTI_STATUS"	VARCHAR2(100)		NOT NULL,
	"NOTI_CONTENT"	CLOB		NOT NULL,
	"NOTI_DATE"	DATE		NOT NULL,
	"NOTI_READ_YN"	VARCHAR2(100)		NOT NULL,
...
SELECT BO_NO, EMP_NO , BO_STATUS,
		...
		BO_LIKE_COUNT, BO_VIEW_COUNT, BO_DEL_YN
...
INSERT INTO NOTI(NOTI_NO, EMP_NO, ORIGIN_NO,
				...
				REG_ID, REG_DATE, MOD_ID, MOD_DATE)
	VALUES('문서번호', 'EE001', '문서번호',
			'3', '결재 문서 반려 - 기안서2가 반려되었습니다', SYSDATE,
...
SELECT COALESCE(NOTI_READ_YN, 'N') 
	FROM NOTI
	WHERE EMP_NO = 'AA001'
		AND ORIGIN_NO = 'C';
	
SELECT COALESCE(NOTI_READ_YN, 'Y') 
    FROM NOTI
    WHERE EMP_NO = 'AA001'
      AND ORIGIN_NO = 'B';
...
SELECT COALESCE(
    (SELECT NOTI_READ_YN 
     FROM NOTI
     WHERE EMP_NO = 'AA001'
       AND ORIGIN_NO = 'B'), 'Y') AS NOTI_READ_YN
	FROM dual;
	
UPDATE NOTI
	SET NOTI_READ_YN = 'Y'
	WHERE EMP_NO = 'AA001'
		AND ORIGIN_NO LIKE 'B___';
...
(SELECT EMP_NO , EMP_NAME , EMP_AUTH 
	FROM EMP
