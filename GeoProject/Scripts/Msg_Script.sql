...
-- 쪽지 읽기 (update)
UPDATE GEO.MSG
	SET MSG_RECV_READ_YN='Y', MOD_ID='HYUN', MOD_DATE=SYSDATE
	WHERE MSG_NO='9';

-- 받은 쪽지 삭제 (update)
UPDATE GEO.MSG
	SET MSG_RECV_DEL_YN='Y', MOD_ID='HYUN', MOD_DATE=SYSDATE
	WHERE MSG_NO IN ('9','40');
-- 보낸 쪽지 삭제 (update)
UPDATE GEO.MSG
	SET MSG_SEND_DEL_YN='Y'	, MOD_ID='HYUN', MOD_DATE=SYSDATE
	WHERE MSG_NO IN ('9','40');
